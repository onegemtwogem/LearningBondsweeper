...
class MinesweeperCountry(object): 
    ...
    def nameCountry(self, canvas): #CITE?
        ...
        if response == None: 
            return None
        else: return response####### fix cancel button

    def getWeightedOptions(self, canvas, weight): #model RECURSION
        for country in self.neighbors:
            canvas.data.weightedOptions += weight*[country]
            if (country not in canvas.data.weightedOptions) and (weight > 1): #base case(s)
...
def redrawAll(canvas, x=0, y=0): #view
    canvas.delete(ALL)
    ...
    if (canvas.data.isGameOver == True) and (canvas.data.youWon == True):
        canvas.create_rectangle(0.15*canvas.data.width, 0.05*canvas.data.height, 
            0.8*canvas.data.width, 0.3*canvas.data.height, fill = "white", width = 0)
...
def removeExcessWhitespace(rawString):
...
def loadMinesweeperBoard(canvas): 
    ...
    if canvas.data.reviewCountries == False:
        ...
        canvas.data.uncoveredCountries = [] #to add countries that get uncovered#model



def init(canvas):
    canvas.data.numberMines = 5
